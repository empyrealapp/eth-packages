...
class DeFiAnalytics:
    ...
    async def analyze_network_status(self):
        ...
        for network, block in zip(self.networks, blocks):
            print(f"{network.__name__:>10}: Block {block.number:,} "
                  f"({datetime.fromtimestamp(block.timestamp).strftime('%H:%M:%S')})")
    
    async def analyze_token_distribution(self, token_address: HexAddress, network_class):
        """
        ...
        transfer_count = 0
        async for event in TransferEvent[network_class].set_filter(
        ...
        if large_transfers:
            ...
            for sender, recipient, amount in large_transfers[:5]:
                print(f"  {amount:,.2f} {symbol}: {sender[:10]}... → {recipient[:10]}...")
    ...
    async def _monitor_network_events(self, network_class, token_address: HexAddress, duration: int):
        ...
        try:
            ...
            async for event in TransferEvent[network_class].set_filter(
                ...
                print(f"[{timestamp}] {network_class.__name__}: "
                      f"{amount:,.2f} {symbol} "
    ...
    async def run_comprehensive_analysis(self):
        ...
        try:
            ...
            await self.analyze_token_distribution(
                self.monitored_tokens[Ethereum], 
                Ethereum
            )
