...
from eth_rpc.utils.encoding import convert_base_model
...
def test_convert_base_model():
    ...
    assert convert_base_model(B) == ['(uint256,string,bool[])[]', 'A', 'bool']
    assert convert_base_model(C) == [
        '((uint256,string,bool[]),((uint256,string,bool[])[],A,bool),((uint256,string,bool[])[],A,bool)[])',
        '((uint256,string,bool[])[],A,bool)[]',
        'string',
    ]
