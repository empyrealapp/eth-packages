...
class Domain(BaseModel):
    ...
    def hash(self):
        hash = keccak(b"EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)")
        ...
        for name, field_data in self.model_fields.items():
            ...
            if isinstance(type_, GenericAlias):
                ...
                if isclass(arg) and issubclass(arg, EIP712Model):
                    ...
                    encoding = keccak(encode_packed_array(
                        'bytes32', hashed_arr,
                    ))
                    values.append(encoding)
                else:
                    arg_name = self.transform(arg)
                    values.append(keccak(
                        encode_packed_array(arg_name, value)
                    ))
