    ...
    try:
        ...
        if isinstance(abi, dict):
            try:
                ...
                raise ValueError("Invalid ABI format: expected 'abi' key in JSON object or direct ABI array")
        ...
        click.echo(f"  Functions: {len([f for f in abi if f.get('type') == 'function'])}")
...
def explorer(
    ...
    try:
        ...
        generated_code = codegen_cmd(abi, contract_name, full_struct_names=full_struct_names)
