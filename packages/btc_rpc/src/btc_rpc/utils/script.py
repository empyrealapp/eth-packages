# FROM: https://github.com/petertodd/python-bitcoinlib/blob/master/bitcoin/core/script.py

from io import BytesIO

import struct

from . import _bignum, core
...
class CScript(bytes):
    ...
    def __coerce_instance(cls, other):
        ...
        if isinstance(other, CScriptOp):
            ...
            if 0 <= other <= 16:
                ...
                other = CScriptOp.encode_op_pushdata(_bignum.bn2vch(other))
    ...
    def is_p2sh(self):
        ...
        """
        return len(self) == 23 and self[0] == OP_HASH160 and self[1] == 0x14 and self[22] == OP_EQUAL
    ...
    def to_p2sh_scriptPubKey(self, checksize=True):
        ...
        return CScript([OP_HASH160, core.Hash160(self), OP_EQUAL])
...
class CScriptWitness(ImmutableSerializable):
    ...
    def __repr__(self):
        return "CScriptWitness(" + ",".join("x('%s')" % core.b2x(s) for s in self.stack) + ")"
...
def RawSignatureHash(script, txTo, inIdx, hashtype):  # noqa: C901
    ...
    txtmp = core.CMutableTransaction.from_tx(txTo)
...
def SignatureHash(script, txTo, inIdx, hashtype, amount=None, sigversion=SIGVERSION_BASE):
    ...
    if sigversion == SIGVERSION_WITNESS_V0:
        ...
        if not (hashtype & SIGHASH_ANYONECANPAY):
            ...
            hashPrevouts = core.Hash(serialize_prevouts)
        ...
        if (
            ...
            hashSequence = core.Hash(serialize_sequence)

        if (hashtype & 0x1F) != SIGHASH_SINGLE and (hashtype & 0x1F) != SIGHASH_NONE:
            serialize_outputs = bytes()
            for o in txTo.vout:
                serialize_outputs += o.serialize()
            hashOutputs = core.Hash(serialize_outputs)
